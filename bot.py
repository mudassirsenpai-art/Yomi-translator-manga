...
APPEAR_BOOL_HINTS = {
    "match_original_style": (
        "Reads the original text's colour, outline colour and thickness, glow "
        "colour and font size from the page — speech bubbles and outside text "
        "both — then renders the translation in that same style, fitted to the "
        "original bubble/region size.\n\n"
...
async def handle_callbacks(client, query: CallbackQuery):
    ...
    if data == "appear_reset_all":
        for field in (
            ...
            "match_original_style", "match_original_style_tolerance",
        ):
            cfg[field] = None
        await save_user_config(user_id)
