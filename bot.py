...
def recompose_tiled_page(translated_dir, page_idx, manifest_entry, cfg=None):
    ...
    for p in translated_tile_paths:
        try:
            ...
            pass
    # Note: seam_ys (all tile-join rows) is returned alongside flagged_seams
    # (only the ones that looked duplicated) purely so callers that build a
    # PDF can do their own hairline-seam cleanup on a copy later. This does
    # NOT change out_path / the saved image — CBZ/ZIP/Raw output is untouched.
    return (out_path, flagged_seams, seam_ys)
...
async def execute_manga_pipeline(client, status_msg: Message, user_id: int):
    ...
    for file_idx, source_message in enumerate(files, start=1):
        ...
        page_seams_by_filename = {}  # only populated for tiled pages; used later for PDF-only seam cleanup
        if tile_manifest:
            ...
            for page_idx, manifest_entry in tile_manifest.items():
                ...
                if result is None:
                    ...
                    continue
                result_path, flagged_seams, all_seam_ys = result
                if flagged_seams:
                    duplicate_suspected_pages.append(page_idx)
                page_seams_by_filename[os.path.basename(result_path)] = all_seam_ys
        ...
        try:
            if cfg['output_format'] == 'img':
                ...
                output_path = package_output(file_translated_dir, job_root, file_idx, cfg['output_format'], page_seams_by_filename=page_seams_by_filename)
...
def _clean_tile_seams_for_pdf(img, seam_ys, probe_px=6, scan_px=14, dark_thresh=40, bright_thresh=235):
    """
    PDF-only cleanup: given a PIL RGB image and the known tile-join Y rows
    (from recompose_tiled_page), inpaint any hairline dark/bright seam found
    right at those rows. Returns a NEW image; never modifies img in place and
    never touches the file on disk — CBZ/ZIP/Raw continue to use the
    original, un-cleaned image exactly as before.
    """
    if not seam_ys:
        return img
    try:
        ...
        return img  # optional dependency missing; ship the PDF uncleaned rather than fail the job
    ...
    for seam_y in seam_ys:
        lo = max(0, seam_y - scan_px)
        hi = min(h, seam_y + scan_px)
        if hi <= lo:
            continue
        band = arr[lo:hi, ::5]  # sample columns for speed
        row_means = band.mean(axis=(1, 2))

        anomaly_rows = [
            i for i, m in enumerate(row_means)
            if m < dark_thresh or m > bright_thresh
        ]
        if not anomaly_rows:
            continue

        a_top = lo + min(anomaly_rows)
        a_bottom = lo + max(anomaly_rows)
        pad = probe_px
        mask[max(0, a_top - pad):min(h, a_bottom + 1 + pad), :] = 255
        touched = True
    ...
    fixed_rgb = cv2.cvtColor(inpainted, cv2.COLOR_BGR2RGB)
    return Image.fromarray(fixed_rgb)


def package_output(source_dir, job_root, file_idx, output_format, page_seams_by_filename=None):
    archive_base = os.path.join(job_root, f"output_{file_idx:03d}")
    if output_format in ("zip", "cbz"):
        # CBZ/ZIP ship the original recomposed images exactly as produced —
        # no seam cleanup here, since readers already handle these joins fine.
        ...
        page_seams_by_filename = page_seams_by_filename or {}
        try:
            from PIL import Image
            imgs = []
            for p in images:
                ...
                seam_ys = page_seams_by_filename.get(p.name)
                if seam_ys:
                    # Only PDF gets the hairline-seam cleanup, and only on this
                    # in-memory copy — the original file on disk (used for
                    # CBZ/ZIP/Raw) is never modified.
                    im = _clean_tile_seams_for_pdf(im, seam_ys)
                imgs.append(im)
            if imgs:
