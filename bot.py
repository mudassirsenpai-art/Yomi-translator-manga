...
def default_config():
    return {
        ...
        "match_original_style": None, "match_original_style_tolerance": None,
        "detect_glow": None,
        "confidence": None, "conjoined_confidence": None, "panel_confidence": None,
        "iou_duplicate_threshold": None,
        ...
        "osb_sdxl_guidance_scale": None, "osb_sdxl_strength": None,
        ...
        "osb_min_side_pixels": None, "osb_model_variant": None
    }
...
def kb_appearance_menu(cfg):
    mos = cfg.get("match_original_style")
    mos_label = "Original/Default (Off)" if mos is None else ("✅ On" if mos else "❌ Off")
    dg = cfg.get("detect_glow")
    dg_label = "Original/Default (Off)" if dg is None else ("✅ On" if dg else "❌ Off")
    rows = [
        ...
        [InlineKeyboardButton(f"🎨 Copy Original Text Style: {mos_label}", callback_data="appear_bool_match_original_style")],
        [InlineKeyboardButton(f"✨ Detect Glow/Halo: {dg_label}", callback_data="appear_bool_detect_glow")],
...
def kb_appear_font_menu(cfg):
    ...
    style_tol = _appear_val_label(cfg, "match_original_style_tolerance", "0.25")
    ...
    rows = [
        ...
        [InlineKeyboardButton(f"🎯 Copy-Style Size Tolerance: {style_tol}", callback_data="appear_field_match_original_style_tolerance")],
...
APPEAR_BOOL_LABELS = {
    ...
    "match_original_style": ("Copy Original Text Style", "menu_appearance"),
    "detect_glow": ("Detect Glow/Halo", "menu_appearance"),
}

# Extra explanation shown above the On/Off keyboard for settings whose effect is
# not obvious from the name alone.
APPEAR_BOOL_HINTS = {
    "match_original_style": (
        "Reads the original text's colour and font size from the page — "
        "speech bubbles and outside text both — then renders the "
        "translation in that same style, fitted to the original "
        "bubble/region size.\n\n"
        "Anything that can't be measured cleanly falls back to the normal "
        "settings, so a page never breaks because of this."
    ),
    "detect_glow": (
        "Checks whether the original text had a soft glow/halo around it "
        "(e.g. a coloured blur behind title-card lettering) and, if so, "
        "reproduces that glow's colour and size behind the translation.\n\n"
        "This is separate from the fixed Outline Width setting: outline "
        "always draws the same hard-edged stroke everywhere, while this "
        "only draws a halo where the original page actually had one, and "
        "matches its colour automatically."
    ),
}
...
VALID_CHOICES = {
    ...
    "osb_inpainting_method": {"flux_klein_9b", "flux_klein_4b", "flux_kontext", "sdxl", "lama", "lama_large", "opencv", "none"},
...
FIELD_REGISTRY = {
    ...
    'osb_inpainting_method': {'group': 'osb', 'label': '🎨 OSB Inpainting Method', 'vtype': 'val', 'argtype': 'str', 'default': 'flux_klein_4b', 'choices': ['flux_klein_9b', 'flux_klein_4b', 'flux_kontext', 'sdxl', 'lama', 'lama_large', 'opencv', 'none'], 'hint': None},
    ...
    'osb_sdxl_guidance_scale': {'group': 'osb', 'label': '🎯 OSB SDXL Guidance Scale', 'vtype': 'val', 'argtype': 'float', 'default': 6.5, 'choices': None, 'hint': 'decimal, e.g. 6.5 (SDXL only; lower = cleaner at low steps)'},
    'osb_sdxl_strength': {'group': 'osb', 'label': '💪 OSB SDXL Strength', 'vtype': 'val', 'argtype': 'float', 'default': 0.99, 'choices': None, 'hint': 'decimal 0-1, e.g. 0.99 (SDXL only; how much art to regenerate)'},
...
FIELD_GROUPS = {
    ...
    'osb': ['osb_inpainting_method', 'osb_flux_backend', 'osb_flux_low_vram', 'osb_flux_sdcpp_cache_mode', 'osb_flux_sdcpp_diffusion_quant', 'osb_flux_sdcpp_text_encoder_quant', 'osb_flux_upscale_small_crops', 'osb_flux_group_regions', 'osb_sfx_skip_inpaint', 'osb_flux_steps', 'osb_flux_luminance_correction', 'osb_flux_residual_threshold', 'osb_sdxl_guidance_scale', 'osb_sdxl_strength', 'osb_seed', 'osb_max_font_size', 'osb_min_font_size', 'osb_use_ligatures', 'osb_outline_width', 'osb_line_spacing', 'osb_use_subpixel', 'osb_font_hinting', 'osb_bbox_expansion', 'osb_render_expansion_narrow', 'osb_render_expansion_tiny', 'osb_render_expansion_aspect_threshold', 'osb_render_expansion_area_threshold', 'osb_text_box_proximity_ratio', 'osb_confidence', 'osb_filter_page_numbers', 'osb_page_filter_margin', 'osb_page_filter_min_area', 'osb_min_area_ignore_ratio', 'osb_min_side_pixels'],
}
...
def kb_detection_menu(cfg):
    ...
    iou_dup = _detect_val_label(cfg, "iou_duplicate_threshold", "0.7")
    seg = _detect_val_label(cfg, "seg_model", "yolo")
    bubble_model = _detect_val_label(cfg, "bubble_detector_model", "yolo_1")
    osb_variant_raw = cfg.get("osb_model_variant")
    if osb_variant_raw is None:
        osb_variant = "Original/Default (auto: manga/webtoon by content type)"
    else:
        osb_variant = osb_variant_raw
    ...
    rows = [
        ...
        [InlineKeyboardButton(f"🪄 Duplicate Box IoU Threshold: {iou_dup}", callback_data="detect_field_iou_duplicate_threshold")],
        ...
        [InlineKeyboardButton(f"📖 OSB Model Variant: {osb_variant}", callback_data="detect_osbvariant_open")],
...
def kb_osb_variant_select(cfg):
    current = cfg.get("osb_model_variant")
    rows = []
    for opt in ("manga", "webtoon"):
        mark = "✅ " if current == opt else ""
        label = f"{opt} (deepghs YOLOv12)" if opt == "manga" else f"{opt} (ogkalu YOLOv8, tuned for long-strip)"
        rows.append([InlineKeyboardButton(f"{mark}{label}", callback_data=f"osbvariantset_{opt}")])
    rows.append([InlineKeyboardButton(f"{'✅ ' if current is None else ''}Original/Default (auto by content type)", callback_data="osbvariantset_default")])
    rows.append([InlineKeyboardButton("🔙 Back", callback_data="menu_detection")])
    return InlineKeyboardMarkup(rows)
...
async def handle_callbacks(client, query: CallbackQuery):
    ...
    if data.startswith("appear_field_"):
        ...
        pretty = {
            ...
            "match_original_style_tolerance": "Copy-Style Size Tolerance (0.0-1.0)",
        }.get(field, field)
    ...
    if data.startswith("appear_bool_"):
        ...
        hint = APPEAR_BOOL_HINTS.get(field)
        body = f"⚙️ **{label}:**" + (f"\n\n{hint}" if hint else "")
        await safe_edit(query.message, body, reply_markup=kb_appear_bool_select(cfg, field))
        return
    ...
    if data == "appear_reset_all":
        for field in (
            ...
            "match_original_style", "match_original_style_tolerance", "detect_glow",
        ):
            cfg[field] = None
        await save_user_config(user_id)
    ...
    if data == "menu_detection":
        await safe_edit(
            ...
            "• **OSB Model Variant**: manga vs webtoon detector for outside-"
            "speech-bubble text; auto-follows Content Type unless overridden.\n"
            "• **Duplicate Box IoU Threshold**: two detected bubbles closer "
            "than this overlap (IoU) are treated as duplicates and the "
            "weaker one is dropped. Raise it (e.g. 0.85) if adjacent-but-"
            "distinct bubbles/captions are being silently discarded.\n"
    ...
    if data.startswith("detect_field_"):
        ...
        pretty = {
            ...
            "iou_duplicate_threshold": "Duplicate Box IoU Threshold (0.3-0.95)",
        }.get(field, field)
    ...
    if data == "manual_translations_retry":
        await safe_answer(query, "Reply with your corrected JSON file")
        job = manual_jobs.get(user_id)
        awaiting_reply[user_id] = {
            "type": "manual_translations_reply",
            "extra": {"reply_to_message_id": job.get("combined_json_message_id") if job else None},
        }
        return
    ...
    if data == "detect_osbvariant_open":
        await safe_edit(
            query.message,
            "📖 **OSB Model Variant**\n\n"
            "Which outside-speech-bubble text detection model to use. "
            "`manga` uses the deepghs YOLOv12 model (default); `webtoon` "
            "uses the ogkalu YOLOv8 model tuned for long-strip manhwa. "
            "By default this auto-follows your Content Type (manhwa → "
            "webtoon, manga → manga); pick one here to override that.",
            reply_markup=kb_osb_variant_select(cfg)
        )
        return

    if data.startswith("osbvariantset_"):
        choice = data.split("_", 1)[1]
        cfg["osb_model_variant"] = None if choice == "default" else choice
        await save_user_config(user_id)
        await safe_answer(query, "OSB model variant updated")
        await safe_edit(query.message, "📖 **OSB Model Variant:**", reply_markup=kb_osb_variant_select(cfg))
        return
    ...
    if data == "detect_reset_all":
        for field in (
            "confidence", "conjoined_confidence", "panel_confidence", "iou_duplicate_threshold", "seg_model",
            "conjoined_detection", "bubble_detector_model", "osb_model_variant", "ocr_method", "translation_mode",
        ):
            cfg[field] = None
...
async def handle_reply_capture(client, message: Message):
    ...
    if kind == "appear_field":
        ...
        pretty = {
            ...
            "match_original_style_tolerance": "Copy-Style Size Tolerance",
        }.get(field, field)
        font_fields = {"min_font_size", "max_font_size", "line_spacing_mult", "match_original_style_tolerance"}
        ...
        # Copy-Style tolerance may legitimately be 0.0 (pin to the measured size exactly).
        zero_ok_fields = {"match_original_style_tolerance"}
        try:
            value = int(text) if is_int_field else float(text)
            if value < 0 or (value == 0 and field not in zero_ok_fields):
                raise ValueError
        except ValueError:
    ...
    if kind == "detect_field":
        ...
        pretty = {
            ...
            "iou_duplicate_threshold": "Duplicate Box IoU Threshold",
        }.get(field, field)
...
CLI_MAPPINGS = {
    ...
    "match_original_style": ("--match-original-style", "bool_true"),
    "match_original_style_tolerance": ("--match-original-style-tolerance", "val"),
    "detect_glow": ("--detect-glow", "bool_true"),
    ...
    "panel_confidence": ("--panel-confidence", "val"), "iou_duplicate_threshold": ("--iou-duplicate-threshold", "val"),
    "seg_model": ("--seg-model", "val"),
    ...
    "osb_sdxl_guidance_scale": ("--osb-sdxl-guidance-scale", "val"), "osb_sdxl_strength": ("--osb-sdxl-strength", "val"),
...
async def execute_manual_pipeline_pass1(client, status_msg: Message, user_id: int):
    ...
    for file_idx, source_message in enumerate(files, start=1):
        ...
        # for that format instead of the manga default; auto-derived from
        # content_type UNLESS the user has explicitly picked one via the
        # OSB Model Variant menu (Detection Settings), which always wins.
        if cfg.get("osb_model_variant") is None:
...
async def execute_manual_pipeline_pass2(client, status_msg: Message, user_id: int, translations_json_path: str):
    ...
    for entry in file_entries:
        ...
        # If the user hand-added any bbox-only "missing bubble" entries to
        # their edited JSON (no bubble_id yet), inject them into this file's
        # checkpoint(s) before rendering. Any entry that still has no
        # bubble_id after this step would otherwise be silently dropped by
        # Pass 2's render loop, so a failure here blocks this file's render
        # entirely rather than rendering an incomplete page.
        injection_report_path = os.path.join(job_root, f"injection_report_file_{file_idx:03d}.json")
        inject_cmd = [
            "python",
            "MangaTranslator/inject_missing_bubbles.py",
            "--combined-json", per_file_json_path,
            "--checkpoint-dir", entry["checkpoint_dir"],
            "--json-report", injection_report_path,
        ]
        try:
            inject_result = await run_engine_subprocess(
                inject_cmd, os.environ.copy(), status_msg,
                progress_dir=None, total_images=1,
                mode_label="Manual Render", stage_label="🧩 Adding missing bubbles",
                file_idx=file_idx, total_files=total_files,
                cancel_check=None, base_pct=35, pct_span=5,
            )
        except Exception as inject_exec_err:
            await safe_edit(
                status_msg,
                f"❌ **File {file_idx}/{total_files}: couldn't run missing-bubble injection:**\n"
                f"`{inject_exec_err}`",
            )
            active_jobs.pop(user_id, None)
            return
        ...
        if inject_result["returncode"] != 0:
            failures = []
            if os.path.exists(injection_report_path):
                try:
                    with open(injection_report_path, "r", encoding="utf-8") as f:
            ...
            if failures:
                lines = [
                    f"• `{fl.get('source_file', '?')}` bbox={fl.get('bbox')}: {fl.get('reason', 'unknown error')}"
                    for fl in failures[:10]
                ]
                more = f"\n...and {len(failures) - 10} more." if len(failures) > 10 else ""
                detail = "\n".join(lines) + more
            else:
                tail = inject_result.get("tail") or "no output captured"
                detail = f"```\n{tail[-800:]}\n```"

            await safe_edit(
                status_msg,
                f"❌ **File {file_idx}/{total_files}: {len(failures) if failures else '?'} bubble(s) "
                f"failed to add:**\n{detail}\n\nFix these entries in your JSON (bbox, coordinates, "
                f"or remove them) and reply with the corrected file.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔁 Fix & Retry", callback_data="manual_translations_retry")]
                ]),
            )
            active_jobs.pop(user_id, None)
            awaiting_reply[user_id] = {
                "type": "manual_translations_reply",
                "extra": {"reply_to_message_id": job.get("combined_json_message_id")},
            }
            return
        ...
        # content_type UNLESS the user has explicitly picked one via the
        # OSB Model Variant menu (Detection Settings), which always wins.
        # Must be set before the CLI_MAPPINGS injection below reads osb_model_variant.
        if cfg.get("osb_model_variant") is None:
            cfg["osb_model_variant"] = "webtoon" if cfg.get("content_type") == "manhwa" else "manga"

        # INJECT ALL CUSTOM PARAMETERS (mirrors execute_manga_pipeline).
        # Without this loop, settings like osb_flux_low_vram, osb_flux_backend,
        # osb_flux_steps etc. are silently ignored during Manual Pass 2 render -
        # only the two hardcoded flags above were ever being sent to main.py.
        for key, config_meta in CLI_MAPPINGS.items():
            flag, val_type = config_meta
            val = cfg.get(key)
            if val is not None and cli_supports_flag(flag):
    ...
    for file_idx, source_message in enumerate(files, start=1):
        ...
        try:
            if cfg['output_format'] == 'img':
                ...
                output_path = package_output(file_translated_dir, job_root, file_idx, cfg['output_format'], source_message=source_message)
                all_translated_outputs.append((file_idx, output_path))
                await client.send_document(
                    source_message.chat.id,
                    document=output_path,
...
def build_output_display_name(source_message, file_idx, ext):
    """Build the final filename shown to the user: '<original name> Translate by Yomi Manga Translator.<ext>'.
    Falls back to a generic name if the original filename isn't available."""
    orig_name = None
    if source_message is not None and getattr(source_message, "document", None) and source_message.document.file_name:
        orig_name = os.path.splitext(source_message.document.file_name)[0]
    if not orig_name:
        orig_name = f"output_{file_idx:03d}"
    safe_name = re.sub(r'[\\/:*?"<>|]', "_", orig_name).strip()
    return f"{safe_name} Translate by Yomi Manga Translator.{ext}"


def package_output(source_dir, job_root, file_idx, output_format, source_message=None):
    ...
    if output_format in ("zip", "cbz"):
        ...
        ext = "cbz" if output_format == "cbz" else "zip"
        if output_format == "cbz":
            cbz_payload = payload.replace(".zip", ".cbz")
            os.rename(payload, cbz_payload)
            payload = cbz_payload
        final_path = os.path.join(job_root, build_output_display_name(source_message, file_idx, ext))
        os.rename(payload, final_path)
        return final_path
        ...
        try:
            ...
            if imgs:
                ...
                final_path = os.path.join(job_root, build_output_display_name(source_message, file_idx, "pdf"))
                os.rename(pdf_path, final_path)
                return final_path
        except Exception:
            pass
        shutil.make_archive(archive_base, "zip", source_dir)
        final_path = os.path.join(job_root, build_output_display_name(source_message, file_idx, "zip"))
        os.rename(f"{archive_base}.zip", final_path)
        return final_path
    else:
        shutil.make_archive(archive_base, "zip", source_dir)
        final_path = os.path.join(job_root, build_output_display_name(source_message, file_idx, "zip"))
